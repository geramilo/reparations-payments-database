...
Script name: rpd3_convert_paid_USD.py
...
--------
This script converts the 'Paid (Total)' amounts in the 
...
1. RPD (V1).xlsx
   - Sheet: DATABASE
   - Columns required:
     * 'Paid (Total)'  → amount to convert
     * 'Currency'      → currency name or ISO code (e.g., USD, EUR)
     * 'Payments conversion year' → year for exchange rate
...
- Adds USD conversions in 'Paid (Total USD)'
...
Things to Remember:
-------------------
- Make sure the script and both input Excel files are in the same folder
- Currency names/codes in the RPD must match the currency map or be ISO 3-letter codes
- Only works for years covered in the exchange rate sheet
- Always run a backup of your original RPD before converting
- USD values are rounded to 2 decimal places
- Missing or invalid data will not be converted, but highlighted in yellow and logged
"""
...
HDR_AMOUNT = "Paid (Total)"
HDR_CURRENCY = "Currency"
HDR_YEAR = "Payments conversion year"
HDR_USD = "Paid (Total USD)"
...
note_ws["A4"] = f"Reference year column: {HDR_YEAR}"
note_ws["A5"] = f"Output column: {HDR_USD}"
note_ws["A6"] = f"Rows updated: {updates}"
note_ws["A8"] = "Detailed log:"
for i, entry in enumerate(log, start=9):
    note_ws[f"A{i}"] = entry
...
print("Check 'Conversion Notes' sheet for full log.")
